...
public class HotelReservationTest {
    @Test
    public void givenValidDateRangeAndRewardCustomer_whenFindingCheapestBestRatedHotel_shouldReturnCorrectHotel() throws Exception {
        ...
        String result = reservation.findCheapestBestRatedHotel(dateRange, customerType);
        assertEquals("Ridgewood, Rating: 5 and Total Rates: $140", result);
    }

    @Test
    public void givenInvalidCustomerType_whenFindingHotel_shouldThrowException() {
        HotelReservation reservation = new HotelReservation();
        ...
        Exception exception = assertThrows(InvalidCustomerTypeException.class, () -> {
            reservation.findCheapestBestRatedHotel(dateRange, customerType);
        });
        assertEquals("Invalid customer type! Please use 'Regular' or 'Reward'.", exception.getMessage());
    }

    @Test
    public void givenInvalidDateFormat_whenFindingHotel_shouldThrowException() {
        HotelReservation reservation = new HotelReservation();
